...
def generate_mcp_cover():
    api_key = get_gemini_key()
    if not api_key:
        print("❌ Chave GEMINI_API_KEY / GOOGLE_API_KEY não encontrada nos arquivos de configuração.")
        return False
        
    print("🎨 Gerando imagem de capa contextual para MCP com Gemini Imagen 3...")
    
    # Contextual prompt following Hermes Cyber-Classicism rules for MCP / Database Integration:
    # "Colunas e pórticos greco-romanos entrelaçados por conduítes de fibra óptica e tabelas holográficas"
    prompt = (
        "High-end duotone artistic composition in the iconic Hermes Agent / Nous Research aesthetic. "
        "A magnificent classical Greco-Roman marble statue bust of Hermes surrounded by monumental classical Corinthian columns "
        "and architectural porticos intricately intertwined with glowing cyan fiber-optic cables and glowing digital data streams. "
        "Floating holographic glowing database tables, data matrices, relational node schemas, and luminous API connection glyphs. "
        ...
        "Fine sculpted marble texture, digital connectivity aura, architectural depth, clean dark background, 16:9 cinematic aspect ratio, "
    ...
    try:
        with urllib.request.urlopen(req) as resp:
            ...
            filename = "mcp-model-context-protocol-hermes.jpg"
